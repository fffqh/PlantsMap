...
<div id="#map">
  <el-container style="height:90vh">
    ...
    <!-- 操控面板 -->
    <el-aside style="height: 100%; width: 34%;">
      <el-tabs v-model="activeTabName" @tab-click="clickTabPage" type="border-card" style="margin-top:20px;">
        <el-tab-pane name="view">
          ...
          <el-row style="margin-top:20px;" >
            <el-button @click="openUserInfo" icon="el-icon-user" circle></el-button>
            <el-button @click="dialogOpen" icon="el-icon-message" circle></el-button>
          </el-row>
          <!-- 历史维护记录展示 -->
          ...
          <el-form ref="form" label-width="80px" style="margin-top:25px;">
            <el-form-item label="植物名称">
              <el-autocomplete
                ...
                @select="handleSelect"
              ></el-autocomplete>
                  <el-tooltip class="item" effect="light" content="去植物仓库浏览植物" placement="top">
                    <el-button style="margin-left:4px;" @click="toHub" icon="el-icon-menu" circle></el-button>
                  </el-tooltip>
            </el-form-item>
            ...
            <el-form-item label="图片上传">
              <el-upload
                ...
                :auto-upload="false"
                :limit="5"
                :action="this.$store.state.host+'/images/map_upload'"
                :data="uploadImgData"
                ref="upload"
                drag multiple 
                with-credentials
  ...
  <el-drawer
    ...
    <div class="demo-drawer__content">
      <el-form :model="mntForm" label-width="80px">
        <el-form-item label="维护类型" >
          <el-select v-model="mntForm.mnt_type" placeholder="请选择维护信息的类型">
            <el-option label="提交更新" value="2"></el-option>
            <el-option label="提交确认" value="3"></el-option>
            <el-option label="提交错误" value="4"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="活动形式">
          <el-input maxlength="250" type="textarea" v-model="mntForm.mnt_desc"></el-input>
        </el-form-item>
        <el-form-item label="图片上传">
          <el-upload
            class="upload-demo"
            ...
            :on-remove ="handleRemove"
            :on-error  ="handleError"
            :on-change ="handleChangeMnt"
            :file-list ="imageListMnt"
            :auto-upload="false"
            :limit="2"
            :action="this.$store.state.host+'/images/msg_upload'"
            :data="uploadImgDataMnt"
            ref="uploadmnt"
            drag multiple 
            with-credentials
            list-type="picture">
            <i class="el-icon-upload"></i>
            <div class="el-upload__text">将图片拖到此处，或<em>点击上传</em></div>
            <div slot="tip" class="el-upload__tip">最多2张图片，只能上传jpg/png文件，且不超过500kb</div>
          </el-upload>
        </el-form-item>
        <el-form-item>
          <el-button type="warning" @click="submitMntInfo" plain  icon="el-icon-check" >提交维护信息</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-drawer>
...
export default {
    ...
    methods: {
      ...
      dialogOpen(){
        ...
        if(this.sltInfo.map_id === null){
          this.$message('请在地图中选择坐标后再打开通信箱')
          return;
        }
        this.dialog=true;
      },
      clickTabPage(){
        if(this.activeTabName == 'view'){
          this.chart.setOption({series:[{id:'cur_point', data:null}]});
          this.$notify({
            title: '游览模式小提示',
            message: '点击地图中已有的植物点，您将看到详细信息。',
      ...
      // 向后端请求地图点数据并加入bmap
      getmap() {
        console.log("get allpoints url:",this.$store.state.host+'/maps/allpoints')
        axios.get(this.$store.state.host+'/maps/allpoints').then(
          response => {
            console.log('/maps/allpoints 请求成功了',response.data)
            if(response.data.msg == 'ok'){
              this.$message({'message':'获取地图点成功', 'type':'success'})
          ...
          error => {
            console.log('/maps/allpoints 请求失败了', error.response)
        ...
        axios.post(this.$store.state.host+"/maps/addpoint", this.addInfo).then(
            response => {
              if (response.data.msg == 'ok') {
                ...
                // 更新地图
                this.getmap();
              }else if(response.data.msg == 'not_in_hub'){
                console.log('/maps/addpoint 请求成功了', response.data);
                this.$message({'message':'该植物不在植物仓库中，您可以前往植物仓库添加该植物。', 'type':'warning'});
              }
            },
            error => {
